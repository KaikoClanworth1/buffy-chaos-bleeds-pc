...
static void bridge_MmAllocateContiguousMemoryEx(void)
{
    ...
    if (low && high >= low && (high - low + 1) <= size + 0x1000) {
        ...
        xbox_ContiguousNotePinned(low, size);
...
/* A guest KEVENT the kernel never saw initialised.
 *
 * KeInitializeEvent is an inline macro in the Xbox headers, so a title that
 * builds its events that way (Xbox D3D does: the event CDevice waits on in
 * BlockOnTime) never calls into the kernel before its first KeSetEvent or
 * KeWaitForSingleObject. Without a shadow the guest pointer was handed to
 * Windows as a HANDLE, every wait failed at once, and D3D's
 * `while (KeWaitForSingleObject(...) != STATUS_SUCCESS)` spun forever.
 *
 * The dispatcher header says what to build: Type 0 is a notification
 * (manual-reset) event, 1 a synchronization (auto-reset) one, and
 * SignalState at +4 the current state. */
/* RECOMP_EVENT_TRACE=0xVA: log every set and wait on one guest object. */
static void ke_event_trace(const char *what, uint32_t guest_va)
{
    static int inited;
    static uint32_t watch;
    static int shown;
    if (!inited) {
        const char *s = getenv("RECOMP_EVENT_TRACE");
        watch = s ? (uint32_t)strtoul(s, NULL, 0) : 0;
        inited = 1;
    }
    if (watch && guest_va == watch && shown++ < 60)
        fprintf(stderr, "  [EVENT] %s 0x%08X type=%u signal=%u tid=%lu t=%llu\n", what, guest_va,
                *(uint8_t *)XBOX_TO_NATIVE(guest_va), BRIDGE_MEM32(guest_va + 4),
                GetCurrentThreadId(), (unsigned long long)GetTickCount64());
}

static HANDLE ke_event_shadow(uint32_t guest_va)
{
    HANDLE h = ke_shadow_lookup(guest_va);
    uint8_t type;

    if (h || guest_va < 0x10000u)
        return h;
    type = *(uint8_t *)XBOX_TO_NATIVE(guest_va);
    if (type > 1)
        return NULL;
...
static void bridge_KeSetEvent(void)
{
    ...
    ke_event_trace("set ", guest_va);
    h = ke_event_shadow(guest_va);
    if (!h)
...
static void bridge_KeWaitForSingleObject(void)
{
    ...
    ke_event_trace("wait", object);
    h = ke_event_shadow(object);
    if (!h)
...
static int kernel_run_dpc(uint32_t dpc_va, uint32_t arg1, uint32_t arg2)
{
...

    {
        ...
        g_esp -= 4; BRIDGE_MEM32(g_esp) = 0;
        fn();
        if (g_esp != esp0) {                  /* a routine popped the wrong amount */
            static int shown;
            if (shown++ < 5)
                fprintf(stderr, "  [KERNEL] DPC 0x%08X esp imbalance %d\n", routine, (int)(g_esp - esp0));
            g_esp = esp0;
        }
    }
    return 1;
...
static void bridge_KeInsertQueueDpc(void)
{
    ...
    /* Inserts come from several threads (the NV2A ISR on the timer thread,
     * DirectSound's own timer/DPC paths), so the ring is locked: an
     * unsynchronised insert could lose a DPC, and losing D3D's GPU DPC leaves
     * PMC_INTR_EN masked forever -- every later GPU interrupt is declined. */
...
static int kernel_raise_interrupt(uint32_t vector)
{
...

    {
        ...
        if (g_esp != esp0) {
            static int shown;
            if (shown++ < 5)
                fprintf(stderr, "  [KERNEL] ISR 0x%08X esp imbalance %d\n", routine, (int)(g_esp - esp0));
            g_esp = esp0;
        }
    }
    return (int)(g_eax & 1u);
...
extern volatile LONG g_nv2a_irq_hold;
extern void xbox_Nv2aPcrtcIntrSet(uint32_t value);
/* Set while a PGRAPH interrupt is latched (kernel_gr_service): vblank
 * delivery must not drop the register hold under it. */
static volatile LONG s_gr_latched_any;
...
static void kernel_vblank_tick(void)
{
    ...
    /* Hold the status registers for the ISR *and* its DPC: D3D's DPC re-reads
     * PMC_INTR_0 to decide what to service, and its VBlank handler then waits
     * for the PCRTC bit to drop after acknowledging (W1C, nv2a_w1c_veh). */
    InterlockedExchange(&g_nv2a_irq_hold, 1);
    xbox_Nv2aPcrtcIntrSet(NV2A_PCRTC_INTR_VBLANK);
    ...
    xbox_Nv2aPcrtcIntrSet(0);
    BRIDGE_MEM32(XBOX_NV2A_REG_BASE + NV2A_PMC_INTR_0)   &= ~NV2A_PMC_INTR_PCRTC;
    if (!s_gr_latched_any)
        InterlockedExchange(&g_nv2a_irq_hold, 0);
}

static void kernel_drain_dpcs(void);

/* ── PGRAPH software-method interrupts ──────────────────────
 *
 * Xbox D3D reports GPU progress through NV097_NO_OPERATION with a non-zero
 * parameter: on hardware the GPU latches method and parameter into
 * PGRAPH_TRAPPED_ADDR/DATA, raises PGRAPH_INTR_ERROR and stalls the pusher
 * until software clears it. D3D's ISR queues its DPC, whose
 * CMiniport::ServiceGrInterrupt sees method 0x100 and calls
 * CMiniport::SoftwareMethod(data) -- which completes fences, signals the
 * event D3D::BlockOnTime sleeps on, and processes flips. Without it a title
 * that ever falls far enough behind to sleep instead of poll (Buffy, on its
 * first Swap) never wakes.
 *
 * The pushbuffer puller (nv2a_pb_scan.c) posts the request and blocks; the
 * timer thread, which owns a guest stack and TIB, delivers it. */
#define NV2A_PGRAPH_INTR_REG      0x00400100u
...
extern volatile LONG g_nv2a_irq_hold;     /* xbox_memory_layout.c */
extern void xbox_Nv2aPgraphIntrSet(uint32_t value);
static volatile LONG s_gr_pending;
static uint32_t s_gr_addr, s_gr_data;
static HANDLE s_gr_done;
static HANDLE s_gr_wake;  /* wakes the timer thread for a posted software method */

int xbox_Nv2aSoftwareMethod(uint32_t subch, uint32_t method, uint32_t data)
{
    if (!xbox_GetConnectedInterrupt(NV2A_VECTOR))
        return 0;
    if (!s_gr_done)
        s_gr_done = CreateEventA(NULL, FALSE, FALSE, NULL);
    if (!s_gr_wake)
        s_gr_wake = CreateEventA(NULL, FALSE, FALSE, NULL);
    s_gr_addr = (subch << 16) | (method & 0x1FFCu);
    s_gr_data = data;
...
/* Optional title hook: service a software method by calling the title's own
 * handler directly (e.g. D3D's CMiniport::SoftwareMethod) instead of going
 * through ISR -> DPC. Receives the method, the data and the context the title
 * registered with KeConnectInterrupt (D3D's CMiniport). Runs on the timer
 * thread, which has a guest stack. */
static void (*s_sw_method_hook)(uint32_t method, uint32_t data, uint32_t isr_context);

void xbox_SetSoftwareMethodHook(void (*hook)(uint32_t, uint32_t, uint32_t))
{
    s_sw_method_hook = hook;
}

static void kernel_gr_service(void)
{
    if (s_sw_method_hook && InterlockedCompareExchange(&s_gr_pending, 0, 0)) {
        uint32_t kint = xbox_GetConnectedInterrupt(NV2A_VECTOR);
        s_sw_method_hook(s_gr_addr & 0x1FFCu, s_gr_data, kint ? BRIDGE_MEM32(kint + 4) : 0);
        kernel_drain_dpcs();
        InterlockedExchange(&s_gr_pending, 0);
        SetEvent(s_gr_done);
        return;
    }
    static int latched;
    ...
    if (!InterlockedCompareExchange(&s_gr_pending, 0, 0))
        return;
    if (!latched) {
        InterlockedExchange(&g_nv2a_irq_hold, 1);
        BRIDGE_MEM32(XBOX_NV2A_REG_BASE + NV2A_PGRAPH_TRAPPED_ADDR) = s_gr_addr;
        BRIDGE_MEM32(XBOX_NV2A_REG_BASE + NV2A_PGRAPH_TRAPPED_DATA) = s_gr_data;
        BRIDGE_MEM32(XBOX_NV2A_REG_BASE + NV2A_PGRAPH_NSOURCE)      = 1;
        xbox_Nv2aPgraphIntrSet(NV2A_PGRAPH_INTR_ERROR);   /* raised by the GPU, not a W1C write */
        BRIDGE_MEM32(XBOX_NV2A_REG_BASE + NV2A_PMC_INTR_0)         |= NV2A_PMC_INTR_PGRAPH;
        latched = 1;
        retries = 0;
        s_gr_latched_any = 1;
    }

    /* Level-triggered, as on hardware. D3D masks GPU interrupts
     * (PMC_INTR_EN = 0) around its own critical sections and its ISR then
     * declines; the interrupt stays pending and is taken once unmasked. A
     * one-shot delivery lost exactly the wake-up BlockOnTime was sleeping on.
     * Retried every timer tick while the pusher stays stalled behind it. */
    claimed = kernel_raise_interrupt(NV2A_VECTOR);
    kernel_drain_dpcs();
    if (claimed == 0 && retries++ < 5000)
        return;
    {
        static unsigned n;
        if (n++ < 5)
            fprintf(stderr, "  [NV2A] software method 0x%04X data 0x%08X -> ISR %s after %u retries\n",
                    s_gr_addr & 0x1FFCu, s_gr_data,
                    claimed < 0 ? "not callable" : claimed ? "claimed it" : "declined it",
                    retries);
    }
    latched = 0;
    s_gr_latched_any = 0;
    xbox_Nv2aPgraphIntrSet(0);
    BRIDGE_MEM32(XBOX_NV2A_REG_BASE + NV2A_PMC_INTR_0)      &= ~NV2A_PMC_INTR_PGRAPH;
