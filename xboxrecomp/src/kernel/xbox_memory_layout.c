...
uint32_t g_xbox_contig_size = 64u * 1024u * 1024u;

void xbox_SetContigSize(uint32_t bytes)
{
    g_xbox_contig_size = bytes;
}
...
/* Set when the pushbuffer puller (nv2a_pb_scan.c) runs: it owns DMA_GET and
 * executes the stream, so the service thread only answers handshakes. */
static int g_pb_fifo_owned;
/* Set by kernel_bridge.c while it delivers a PGRAPH interrupt. */
volatile LONG g_nv2a_irq_hold;
extern volatile uint32_t g_pb_idle_put;   /* nv2a_pb_scan.c */
...
static const struct { uint32_t offset; uint32_t busy_mask; } NV2A_ACK[] = {
    ...
    /* PGRAPH_INTR (0x400100) is not held here: it is write-1-to-clear, see
     * nv2a_w1c_veh. Zeroing it behind the ISR's back hid real interrupts. */
    /* PCRTC_INTR_0 (0x600100) is write-1-to-clear too: nv2a_w1c_veh. */
    /* PGRAPH_STATUS: nothing here is ever busy. D3D's ServiceGrInterrupt
     * spins on it reading non-zero before handling a context switch. */
    { 0x400700, 0xFFFFFFFFu },  /* PGRAPH_STATUS */
};
...
/* ── when the GPU threads may sleep ───────────────────────────────────────
 *
 * The pushbuffer puller (nv2a_pb_scan.c) and the register thread below
 * answer handshakes the title spins on: KickOff sets a flush bit and loops
 * until it clears, IsBusy is polled in loops. Both threads used to spin
 * forever (SwitchToThread / Sleep(0)) so the answer came at once -- two whole
 * CPU cores, all the time, for a 60 fps game.
 *
 * Now they spin only while the GPU is "hot": the title is inside one of those
 * waits (xbox_gpu_hurry, called by the title's wrappers around them), or
 * there was GPU work in the last 2 ms (xbox_gpu_poke). Otherwise each sleeps
 * on its own event with a 1 ms timeout, and a wait starting rings both.
...
static void gpu_idle_init(void)
{
    LARGE_INTEGER f;
    if (g_gpu_spin >= 0)
        return;
    QueryPerformanceFrequency(&f);
    g_qpc_2ms = f.QuadPart / 500;
...
void xbox_gpu_poke(void)
{
    LARGE_INTEGER now;
    QueryPerformanceCounter(&now);
    g_gpu_hot_until = now.QuadPart + g_qpc_2ms;
}

/* The title enters (+1) or leaves (-1) a wait on the GPU. */
void xbox_gpu_hurry(int delta)
{
    gpu_idle_init();
    if (delta > 0) {
        InterlockedIncrement(&g_gpu_hurry);
        SetEvent(g_gpu_bell[0]);
        SetEvent(g_gpu_bell[1]);
    } else {
        InterlockedDecrement(&g_gpu_hurry);
        xbox_gpu_poke();
    }
}

/* Called by an idle GPU thread (`which` 0 puller, 1 register thread). */
void xbox_gpu_idle(int which)
{
    LARGE_INTEGER now;
    gpu_idle_init();
    QueryPerformanceCounter(&now);
    ...
    WaitForSingleObject(g_gpu_bell[which], 1);
}
...
static DWORD WINAPI nv2a_ack_thread(LPVOID param)
{
    ...
    while (!InterlockedCompareExchange(&g_nv2a_ack_stop, 0, 0)) {
        for (size_t i = 0; i < sizeof(NV2A_ACK) / sizeof(NV2A_ACK[0]); i++) {
            ...
            /* An interrupt being delivered must stay visible to the ISR. */
            if (g_nv2a_irq_hold && (NV2A_ACK[i].offset == 0x000100
                                    || NV2A_ACK[i].offset == 0x400100))
                continue;
            /* D3D's flush handshake (KickOff, FlushWCCache): with the puller
             * executing the stream, "flushed" means drained up to PUT. Held
             * back until then, so D3D never gets hundreds of fences ahead of
             * the GPU (it only tracks 64). A 1 s valve prevents a deadlock.
             */
            if (NV2A_ACK[i].offset == 0x100410 && g_pb_fifo_owned && (*r & NV2A_ACK[i].busy_mask)) {
                static DWORD held_since;
                uint32_t cur_put = *(volatile uint32_t *)((char *)regs + NV2A_USER_DMA_PUT) & 0x0FFFFFFFu;
                if (g_pb_idle_put != cur_put) {
                    if (!held_since) held_since = GetTickCount();
                    if (GetTickCount() - held_since < 1000)
                        continue;
                }
                held_since = 0;
            }
         ...
         * only ran when someone happened to also be tracing. */
        {
            ...
            if (put != last_put || (now_ms - last_put_ms) > 2000) {
                /* Survey the segment the title just submitted, once. */
                {
                     ...
                     * guess. */
                    if (!g_pb_fifo_owned && last_put && put > last_put)
        ...
        *(volatile uint32_t *)((uintptr_t)(XBOX_KERNEL_DATA_BASE + KDATA_TICK_COUNT)
        ...
        xbox_gpu_idle(1);  /* spins only while the title waits on the GPU */
    }
    return 0;
}

/* PGRAPH_INTR is write-1-to-clear.
 *
 * D3D's interrupt path acknowledges by writing the pending bits back, and its
 * DPC (CMiniport::Dpc) loops for as long as ServiceGrInterrupt reports
 * PGRAPH_INTR non-zero. Against plain memory the acknowledgement stores the
 * bits again, so the first delivered software-method interrupt never ends.
 *
 * The page holding it (0xFD400000, the PGRAPH register block) is kept
 * read-only. A write faults, is single-stepped with the page writable, and if
 * it touched PGRAPH_INTR the register becomes `old & ~written` -- with the
 * PGRAPH bit dropped from PMC_INTR_0 once nothing is pending, as the hardware
 * derives it. Reads cost nothing; D3D writes this page only from its interrupt
 * path. Other offsets in the page are stored as written. */
#define NV2A_PGRAPH_PAGE   0x400000u
#define NV2A_PGRAPH_INTR   0x400100u
static RECOMP_TLS int      t_w1c_step;       /* 1 = stepping, 2 = stepping a write to INTR */
static RECOMP_TLS uint32_t t_w1c_old;

static void nv2a_pgraph_protect(DWORD prot)
{
    DWORD old;
    VirtualProtect((char *)g_nv2a_memory + NV2A_PGRAPH_PAGE, 0x1000, prot, &old);
}

/* Raw store for the runtime's own interrupt delivery (kernel_bridge.c): the
 * GPU raising a bit is not a software write and must not be W1C'd. */
void xbox_Nv2aPgraphIntrSet(uint32_t value)
{
    if (!g_nv2a_memory)
        return;
    nv2a_pgraph_protect(PAGE_READWRITE);
    *(volatile uint32_t *)((char *)g_nv2a_memory + NV2A_PGRAPH_INTR) = value;
    nv2a_pgraph_protect(PAGE_READONLY);
}

/* Both W1C interrupt-status registers D3D acknowledges: PGRAPH_INTR (the
 * software-method path) and PCRTC_INTR_0 (vblank). Each page's register sits
 * at +0x100, and each block has its own PMC_INTR_0 summary bit. */
static const struct { uint32_t page; uint32_t pmc_bit; } NV2A_W1C[] = {
    { 0x400000u, 1u << 12 },   /* PGRAPH */
    { 0x600000u, 1u << 24 },   /* PCRTC  */
};
static RECOMP_TLS int t_w1c_page;

static void nv2a_w1c_protect(int i, DWORD prot)
{
    DWORD old;
    VirtualProtect((char *)g_nv2a_memory + NV2A_W1C[i].page, 0x1000, prot, &old);
}
...
static LONG CALLBACK nv2a_w1c_veh(PEXCEPTION_POINTERS ep)
{
    ...
    if (er->ExceptionCode == EXCEPTION_ACCESS_VIOLATION
        ...
        char *a = (char *)er->ExceptionInformation[1];
        for (i = 0; i < (int)(sizeof NV2A_W1C / sizeof NV2A_W1C[0]); i++) {
            char *page = (char *)g_nv2a_memory + NV2A_W1C[i].page;
            if (a < page || a >= page + 0x1000)
                continue;
            t_w1c_old = *(volatile uint32_t *)(page + 0x100);
            t_w1c_step = (a >= page + 0x100 && a < page + 0x104) ? 2 : 1;
            t_w1c_page = i;
            nv2a_w1c_protect(i, PAGE_READWRITE);
            ep->ContextRecord->EFlags |= 0x100;      /* trap after one instruction */
            return EXCEPTION_CONTINUE_EXECUTION;
        }
        return EXCEPTION_CONTINUE_SEARCH;
    }
    if (er->ExceptionCode == EXCEPTION_SINGLE_STEP && t_w1c_step) {
        i = t_w1c_page;
        if (t_w1c_step == 2) {
            char *page = (char *)g_nv2a_memory + NV2A_W1C[i].page;
            volatile uint32_t *intr = (volatile uint32_t *)(page + 0x100);
            volatile uint32_t *pmc  = (volatile uint32_t *)((char *)g_nv2a_memory + 0x100);
            *intr = t_w1c_old & ~*intr;
            if (!*intr)
                *pmc &= ~NV2A_W1C[i].pmc_bit;
        }
        t_w1c_step = 0;
        nv2a_w1c_protect(i, PAGE_READONLY);
        return EXCEPTION_CONTINUE_EXECUTION;
    }
    return EXCEPTION_CONTINUE_SEARCH;
}
...
void xbox_Nv2aPcrtcIntrSet(uint32_t value)
{
    if (!g_nv2a_memory)
        return;
    nv2a_w1c_protect(1, PAGE_READWRITE);
    *(volatile uint32_t *)((char *)g_nv2a_memory + 0x600100u) = value;
...
#undef WD_NV2A
    ...
    /* Where the guest thread actually is. A pure CPU spin leaves the guest
    ...
    if (s_watchdog_thread) {
        int k;
        for (k = 0; k < 12; k++) {
            /* The last four samples are of the NV2A service thread: every
             * register handshake and the pushbuffer executor run on it, so a
             * guest spinning on a busy bit is often really that thread stuck. */
            HANDLE th = (k < 8 || !g_nv2a_ack_thread) ? s_watchdog_thread
                                                      : g_nv2a_ack_thread;
            CONTEXT c;
...
BOOL xbox_MemoryLayoutInit(const void *xbe_data, size_t xbe_size)
{
     ...
     */
    {
         ...
         * variable arms it. */
        /* The PUT/GET line itself only with RECOMP_NV2A_TRACE: printed
         * per poll, it grew the player's log by megabytes a session. */
        s_nv2a_trace_log = getenv("RECOMP_NV2A_TRACE") != NULL;
 ...
 *
 * Starts above the low 64 KB of physical memory, which the retail kernel
 * never hands out. XAPI's USB driver (USBD_AllocateMemory) carves its device
 * descriptors straight out of physical page 0 at 0x80000000..0x80000FFF
 * without asking the allocator, so returning that page to the title put
 * Buffy: Chaos Bleeds' 6 MB engine heap underneath the USB device table and
 * the first USB device constructed overwrote the heap's free list. */
#define XBOX_CONTIG_RESERVED_LOW 0x10000u
static uint32_t g_contig_next = XBOX_CONTIG_BASE + XBOX_CONTIG_RESERVED_LOW;
...
/* Ranges a title pinned at a physical address it chose
 * (MmAllocateContiguousMemoryEx with lowest..highest one allocation wide).
 * They sit in the window too, but not below the bump allocator's mark, so
 * the high-water test above misses them. */
static struct { uint32_t lo, hi; } g_contig_pinned[16];
static int g_contig_pinned_n;

void xbox_ContiguousNotePinned(uint32_t phys, uint32_t size)
{
    if (g_contig_pinned_n < 16) {
        g_contig_pinned[g_contig_pinned_n].lo = phys;
        g_contig_pinned[g_contig_pinned_n].hi = phys + size;
        g_contig_pinned_n++;
    }
}

int xbox_ContiguousIsPinned(uint32_t phys)
{
    int i;
    for (i = 0; i < g_contig_pinned_n; i++)
        if (phys >= g_contig_pinned[i].lo && phys < g_contig_pinned[i].hi)
            return 1;
    return 0;
}
