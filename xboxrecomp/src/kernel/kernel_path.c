        ...
        if (sub_dir) {
            ...
            if (base_dir == s_game_dir && remainder_wide[0]) {
                ...
                for (k = s_overlay_count - 1; k >= 0; k--) {
                    WCHAR alt[MAX_PATH];
                    swprintf_s(alt, MAX_PATH, L"%s\\%s", s_overlay[k], remainder_wide);
