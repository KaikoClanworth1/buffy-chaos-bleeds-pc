...
int xa2_submit_samples(const int16_t *samples, int num_samples)
{
    ...
    /* RECOMP_APU_PEAK: is anything audible reaching the output? */
    {
        static int on = -1, peak;
        static DWORD last;
        static unsigned long bufs;
        int k;
        if (on < 0)
            on = getenv("RECOMP_APU_PEAK") != NULL;
        if (on) {
            ...
            if (GetTickCount() - last > 5000) {
                last = GetTickCount();
                fprintf(stderr, "  [APU] output peak %d over %lu buffers\n", peak, bufs);
                peak = 0;
                bufs = 0;
            }
        }
