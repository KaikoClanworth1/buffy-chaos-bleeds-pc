...
#include <stdlib.h>
#if defined(_WIN32)
#include <windows.h>  /* GetTickCount: RECOMP_PAD_SCRIPT */
#endif
...
int usb_gamepad_report(uint8_t *out, int max)
{
    ...
    /* RECOMP_PAD_SCRIPT="secs:BUTTON[,secs:BUTTON...]" presses BUTTON for
     * 300 ms at that many seconds after the first report: unattended runs can
     * get past title screens. BUTTON is START, BACK, A, B, X, Y, UP, DOWN,
     * LEFT or RIGHT. */
    {
        static int inited;
        static struct { unsigned ms; char name[8]; } ev[32];
        static int nev;
        ...
        for (k = 0; k < nev; k++) {
            unsigned long rel = now - t0;
            if (rel >= ev[k].ms && rel < ev[k].ms + 300) {
                const char *b = ev[k].name;
                if (!strcmp(b, "UP"))    out[2] |= 0x01;
                if (!strcmp(b, "DOWN"))  out[2] |= 0x02;
                if (!strcmp(b, "LEFT"))  out[2] |= 0x04;
                if (!strcmp(b, "RIGHT")) out[2] |= 0x08;
                if (!strcmp(b, "START")) out[2] |= 0x10;
                if (!strcmp(b, "BACK"))  out[2] |= 0x20;
                if (!strcmp(b, "A"))     out[4] = 0xFF;
                if (!strcmp(b, "B"))     out[5] = 0xFF;
                if (!strcmp(b, "X"))     out[6] = 0xFF;
                if (!strcmp(b, "Y"))     out[7] = 0xFF;
                return 20;
            }
        }
    }
