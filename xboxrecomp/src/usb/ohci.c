...
typedef struct {
    ...
    volatile uint32_t acks;   /* HcInterruptStatus writes: the driver acknowledging */
...
/* The controller is a bus master: every pointer the driver gives it (HCCA,
 * ED and TD links, buffer pointers) is a PHYSICAL address, which on Xbox is
 * visible at 0x80000000 + p. Physical addresses inside the contiguous region
 * the runtime has handed out are translated; anything else is used as is. */
extern uint32_t xbox_ContiguousAllocatedBytes(void);
static uint32_t phys_va(uint32_t p)
{
    if (p && p < 0x80000000u && p < xbox_ContiguousAllocatedBytes())
...
static DWORD WINAPI ohci_thread(LPVOID unused)
{
    ...
    unsigned waited = 0;
    int plugged = 0;
    uint32_t last_status = 0, last_acks = 0;
    unsigned repeats = 0;
    ...
    for (;;) {
        ...
        /* Stuck means the same status with no acknowledgement in between;
         * StartOfFrame re-raised every tick and cleared every time is a
         * driver counting frames, not a stuck source. */
        if (status == last_status && hc->acks == last_acks) {
            if (++repeats > 200) {
