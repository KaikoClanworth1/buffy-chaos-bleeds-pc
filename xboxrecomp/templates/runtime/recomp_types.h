...
/* frndint rounds by the guest control word's RC field, not the host's mode:
 * the CRT's floor/ceil set RC to down/up around it. */
...
/* Branch at a block boundary, where the lifter cannot see which instruction
 * set the flags (several predecessors each end in their own compare and
 * share one jcc). In functions that have such a branch, every flag setter
 * records its kind in _fk and the branch evaluates the real condition here.
 * Kinds: 1 cmp (_fa/_fb, _fas/_fbs = operands), 2 test (operands), 3
 * arithmetic/logical result (_fa/_fas = result), 4 SSE compare (_fca/_fcb),
 * 5 x87 compare-to-EFLAGS (g_fp_cmp), 6 sahf (_fah = AH).
 * Conditions: 0 o 1 no 2 b 3 ae 4 e 5 ne 6 be 7 a 8 s 9 ns 10 p 11 np
 * 12 l 13 ge 14 le 15 g. */
static inline int recomp_jcc_dyn(int cc, int fk, uint32_t fa, uint32_t fb, int32_t fas, int32_t fbs,
                                 double fca, double fcb, int fpcmp, int cf_in, uint8_t fah)
{
    int zf = 0, cf = cf_in, sf = 0, of = 0, pf = 0;
    switch (fk) {
    case 1: {
        int64_t r = (int64_t)fas - (int64_t)fbs;
        zf = fa == fb; cf = fa < fb; sf = (int32_t)(fa - fb) < 0;
        of = (r < INT32_MIN || r > INT32_MAX) ? 1 : ((fas < fbs) != sf);
        pf = recomp_parity8(fa - fb);
        break;
    }
    case 2:
        zf = (fa & fb) == 0; cf = 0; of = 0; sf = (int32_t)(fa & fb) < 0; pf = recomp_parity8(fa & fb);
        break;
    case 3:
        zf = fa == 0; sf = fas < 0; of = 0; pf = recomp_parity8(fa);
        break;
    case 4: {
        int un = (fca != fca) || (fcb != fcb);
        zf = un || fca == fcb; cf = un || fca < fcb; pf = un; sf = 0; of = 0;
        break;
    }
    case 5:
        zf = fpcmp == 0 || fpcmp == 2; cf = fpcmp == -1 || fpcmp == 2; pf = fpcmp == 2; sf = 0; of = 0;
        break;
    case 6:
        zf = (fah >> 6) & 1; cf = fah & 1; pf = (fah >> 2) & 1; sf = (fah >> 7) & 1; of = 0;
        break;
    default:
        return 0;
    }
