...
def _make_condition(jcc, flag_setter, flag_ops):
    ...
    if flag_setter in ("fcompi", "fcomip", "fucomi", "fucompi",
                        "fucomip", "fcomi", "sahf"):
        # g_fp_cmp: -1 less, 0 equal, 1 greater, 2 unordered. Unordered sets
        # CF = ZF = PF = 1, so jb/jbe/je/jp are taken on NaN and ja/jae/jne/jnp
        # are not.
        fpu_cmp_map = {
            "ja": "(g_fp_cmp == 1)", "jnbe": "(g_fp_cmp == 1)",
            "jae": "(g_fp_cmp == 0 || g_fp_cmp == 1)", "jnb": "(g_fp_cmp == 0 || g_fp_cmp == 1)",
            "jnc": "(g_fp_cmp == 0 || g_fp_cmp == 1)",
            "jb": "(g_fp_cmp == -1 || g_fp_cmp == 2)", "jnae": "(g_fp_cmp == -1 || g_fp_cmp == 2)",
            "jc": "(g_fp_cmp == -1 || g_fp_cmp == 2)",
            "jbe": "(g_fp_cmp != 1)", "jna": "(g_fp_cmp != 1)",
            "je": "(g_fp_cmp == 0 || g_fp_cmp == 2)", "jz": "(g_fp_cmp == 0 || g_fp_cmp == 2)",
            "jne": "(g_fp_cmp == -1 || g_fp_cmp == 1)", "jnz": "(g_fp_cmp == -1 || g_fp_cmp == 1)",
    ...
    if flag_setter in ("comiss", "comisd", "ucomiss", "ucomisd"):
        ...
        a, b = "_fca", "_fcb"
        # comiss/ucomiss set CF, ZF, PF; unordered (NaN) sets all three, so
        # jb/jbe/je/jp are taken on NaN and ja/jae/jne/jnp are not.
        if jcc in ("ja", "jnbe"):
            return f"({a} > {b})", desc
...
class Lifter:
    ...
    _DYN_CC = {"o": 0, "no": 1, "b": 2, "nae": 2, "c": 2, "ae": 3, "nb": 3, "nc": 3,
               "e": 4, "z": 4, "ne": 5, "nz": 5, "be": 6, "na": 6, "a": 7, "nbe": 7,
               "s": 8, "ns": 9, "p": 10, "pe": 10, "np": 11, "po": 11,
               "l": 12, "nge": 12, "ge": 13, "nl": 13, "le": 14, "ng": 14, "g": 15, "nle": 15}

    def _dyn_cond(self, cc_name):
        """Runtime flag evaluation for a condition whose setter is not in view
        (see recomp_jcc_dyn). None when dynamic flags are off or unknown."""
        if not getattr(self, "dyn_flags", False):
            return None
        cc = self._DYN_CC.get(cc_name)
        if cc is None:
            return None
        cf = "_cf" if self.needs_cf else "0"
        return (f"recomp_jcc_dyn({cc}, _fk, _fa, _fb, _fas, _fbs, _fca, _fcb, "
                f"g_fp_cmp, {cf}, _fah)")
    ...
    def _lift_jcc(self, insn):
        ...
        if self.needs_cf:
            if jcc in ("jb", "jnae", "jc"):
                ...
                cond = "!_cf"
        dyn = self._dyn_cond(jcc[1:])
        if dyn:
            cond = dyn
        else:
            # Marks a branch resolved without seeing its flag setter, so the
            # translator re-lifts the function with dynamic flags.
            cond += " /*blindjcc*/"
    ...
    def _lift_setcc(self, insn, ops, m):
        if len(ops) < 1:
            return [f"/* {m}: no operand */"]
        dyn = self._dyn_cond(m[3:])
        return [_fmt_operand_write(ops[0], f"({dyn} ? 1 : 0) /* {m} */" if dyn else f"_flags /* {m} */")]

    def _lift_cmovcc(self, insn, ops, m):
        if len(ops) < 2:
            return [f"/* {m}: bad operands */"]
        src = _fmt_operand_read(ops[1])
        dyn = self._dyn_cond(m[4:])
        return [f"if ({dyn or '_flags'} /* {m} */) {_fmt_operand_write(ops[0], src)}"]
    ...
    def _lift_rep_string(self, insn, m):
        ...
        # Hardware apertures (NV2A at 0xFD000000, MCPX/APU at 0xFE800000) are
        # trapped and emulated one decoded instruction at a time. memcpy there
        # faults inside the CRT on SSE loads the trap decoder cannot step, so
        # copies touching them take the element-wise volatile loop instead.
        # (Buffy: DirectSound reads I3DL2 effect data back out of GP DSP memory.)
        if "movsb" in m:
            return ["if (!g_df) { uint8_t *_d = (uint8_t*)XBOX_PTR(edi),"
                    " *_s = (uint8_t*)XBOX_PTR(esi); uint32_t _n = ecx;",
                    "  if (edi < 0xFD000000u && esi < 0xFD000000u"
                    " && (_d + _n <= _s || _s + _n <= _d)) memcpy(_d, _s, _n);",
                    "  else { uint32_t _i; for (_i = 0; _i < _n; _i++) MEM8(edi + _i) = MEM8(esi + _i); }",
                    "  esi += ecx; edi += ecx; }",
        ...
        if "movsw" in m:
            return ["if (!g_df) { uint8_t *_d = (uint8_t*)XBOX_PTR(edi),"
                    ...
                    "ecx = 0; /* rep movsw */"]
        if "stosb" in m:
            return ["if (!g_df && edi < 0xFD000000u) { memset((void*)XBOX_PTR(edi), (uint8_t)eax, ecx); edi += ecx; }",
    ...
    def _lift_fpu(self, insn, m, ops):
        ...
        if m == "frndint":
            return [f"fp_top() = recomp_frndint(fp_top(), g_fp_control_word); /* frndint */"]
        if m == "fldpi":
...
def lift_basic_block(lifter, bb, flag_state=None):
    ...
    while i < len(insns):
        ...
        if curr.mnemonic == "neg":
            ...
            # MSVC's float-equality idiom is ucomiss; lahf; test ah, 0x44;
            # jnp/jp. AH has to carry the compare's ZF/PF/CF (SF=AF=0, bit 1
            # set) or every such test branches on stale AH -- which, among
            # much else, kept the particle allocator from finding a free slot.
            results = [
                "{ uint32_t _ah = 0x02u;"
        ...
        # Dynamic flags: record what kind of instruction set the flags, for a
        # branch in another block that cannot see it (recomp_jcc_dyn).
        if getattr(lifter, "dyn_flags", False):
            _m = curr.mnemonic
            _kind = None
            if _m in ("cmp", "bsf", "bsr", "cmpxchg", "lock cmpxchg"):
                _kind = 1
            elif _m == "test":
                _kind = 2
            elif _m in ("comiss", "comisd", "ucomiss", "ucomisd"):
                _kind = 4
            elif _m in ("fcompi", "fcomip", "fucomi", "fucompi", "fucomip", "fcomi"):
                _kind = 5
            elif _m == "sahf":
                stmts.append("_fah = (uint8_t)HI8(eax);")
                _kind = 6
            elif _m in ("add", "sub", "and", "or", "xor", "inc", "dec", "neg", "adc", "sbb",
                        "shl", "sal", "shr", "sar", "lock add", "lock sub", "lock inc", "lock dec",
                        "lock and", "lock or", "lock xor", "xadd", "lock xadd"):
                _kind = 3
            if _kind is not None:
                stmts.append(f"_fk = {_kind};")
