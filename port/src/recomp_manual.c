...
void recomp_icall_not_code_log(uint32_t va)
{
    ...
    /* The caller pushed its guest return address before dispatching, so the
     * call site is sitting at [esp]. */
    fprintf(stderr, "[ICALL] target 0x%08X is not code -- skipped %llu time(s) "
                    "(null or wild function pointer, at call #%llu, ret 0x%08X)\n",
            va, (unsigned long long)hits[i],
            (unsigned long long)g_icall_count,
            *(uint32_t *)((uintptr_t)g_esp + g_xbox_mem_offset));
    fflush(stderr);
}
